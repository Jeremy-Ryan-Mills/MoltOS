...
use chronos::thread::{Thread, ThreadId, context::ThreadContext};
...
fn test_block_removes_from_scheduling() {
    chronos::serial_print!("sync::test_block_removes_from_scheduling...\t");
...
fn test_unblock_restores_runnability() {
    chronos::serial_print!("sync::test_unblock_restores_runnability...\t");
    let mut sched = EevdfScheduler::new();
    sched.spawn(Thread::new(dummy));
    let mut bootstrap = ThreadContext::default();
...
fn test_blocked_thread_is_skipped() {
    chronos::serial_print!("sync::test_blocked_thread_is_skipped...\t");
    let mut sched = EevdfScheduler::new();
    sched.spawn(Thread::new(dummy));
    sched.spawn(Thread::new(dummy));
    let mut bootstrap = ThreadContext::default();
...
fn test_unblock_unknown_id_is_noop() {
    chronos::serial_print!("sync::test_unblock_unknown_id_is_noop...\t");
    let mut sched = EevdfScheduler::new();
    sched.spawn(Thread::new(dummy));
    let mut bootstrap = ThreadContext::default();
...
fn test_all_threads_blocked() {
    chronos::serial_print!("sync::test_all_threads_blocked...\t");
    let mut sched = EevdfScheduler::new();
    sched.spawn(Thread::new(dummy));
    sched.spawn(Thread::new(dummy));
    let mut bootstrap = ThreadContext::default();
...
fn test_kmutex_basic_lock_unlock() {
    chronos::serial_print!("sync::test_kmutex_basic_lock_unlock...\t");
...
fn test_kmutex_guard_deref_mut() {
    chronos::serial_print!("sync::test_kmutex_guard_deref_mut...\t");
...
fn test_kmutex_sequential_locks() {
    chronos::serial_print!("sync::test_kmutex_sequential_locks...\t");
    let m = KMutex::new(0u32);
    for i in 0..20u32 {
        let mut g = m.lock();
        *g = i;
